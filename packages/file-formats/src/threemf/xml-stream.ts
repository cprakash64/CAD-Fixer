...
/** Characters of carry that make a fixed-length literal search boundary-proof. */
const DOCTYPE_CARRY = '<!DOCTYPE'.length - 1;
const ENTITY_CARRY = '<!ENTITY'.length - 1;
const EXTERNAL_WORDS = ['SYSTEM', 'PUBLIC'] as const;
const WHITESPACE = /\s/;
...
/** ASCII-only upper-casing: what `/i` does to these letters without the `u` flag. */
function upperAscii(code: number): number {
  return code >= 97 && code <= 122 ? code - 32 : code;
}

/**
 * `describeUnsafeXml`, incrementally.
 *
 * THE RULES ARE THE WHOLE-STRING RULES, NOT A RE-INTERPRETATION of them:
 *
 *   1. `<!DOCTYPE` (any case) anywhere in the PROLOG — everything before the
 *      first `<` that is not followed by `?` or `!` — including inside a
 *      comment there;
 *   2. `<!ENTITY` (any case) anywhere in the WHOLE text;
 *   3. `\b(SYSTEM|PUBLIC)\s+["']` (any case) anywhere in the prolog;
 *
 * reported in that precedence. Rules 1 and 2 are fixed-length literals, so a
 * carry of `length - 1` characters from the previous piece is exactly enough:
 * an occurrence spanning a boundary has at most that many characters before it,
 * and one lying wholly in the carry was already seen. Rule 3 has an unbounded
 * `\s+`, so it is an explicit automaton instead of a carry: word boundary, the
 * word, at least one whitespace character of JavaScript's `\s` class, a quote.
 *
 * NOTHING IS DECIDED UNTIL `finish()`. The whole-string precedence means a
 * DOCTYPE late in a long prolog outranks an ENTITY early in it, so this reports
 * nothing mid-stream; the driver runs it over the WHOLE part before a single
 * element is scanned for meaning — the same guarantee `scanXml` gives.
 */
export class XmlSecurityStream {
  ...
  private doctypeCarry = '';
  private entityCarry = '';
  /** Rule 3: 0 idle, 1 inside a candidate word, 2 after the word in whitespace. */
  private stage = 0;
  private word = 0;
  private matched = 0;
  ...
  public push(text: string): void {
    ...
    if (!this.entity) {
      const window = this.entityCarry + text;
      if (ENTITY.test(window)) this.entity = true;
      this.entityCarry = window.slice(-ENTITY_CARRY);
    }
    if (this.inProlog) this.pushProlog(text);
  }

  public finish(): ImportRefusal | undefined {
    // A `<` as the very last character does not extend the prolog: `prologOf`
    // reads the character after it, finds none, and ends the prolog before it.
    this.pendingLt = false;
    if (this.doctype) return ImportRefusal.XmlDoctypeRefused;
    if (this.entity) return ImportRefusal.XmlEntityRefused;
  ...
  private feedProlog(segment: string): void {
    ...
    if (!this.doctype) {
      const window = this.doctypeCarry + segment;
      if (DOCTYPE.test(window)) this.doctype = true;
      this.doctypeCarry = window.slice(-DOCTYPE_CARRY);
    }
    if (this.external) return;
    for (let at = 0; at < segment.length; at += 1) {
      const code = segment.charCodeAt(at);
      if (this.stage === 2) {
        if (WHITESPACE.test(segment.charAt(at))) {
          this.sawSpace = true;
        ...
        const target = EXTERNAL_WORDS[this.word] ?? '';
        if (upperAscii(code) === target.charCodeAt(this.matched)) {
          this.matched += 1;
          if (this.matched === target.length) {
            this.stage = 2;
            this.sawSpace = false;
          }
